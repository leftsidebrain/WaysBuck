...
import { useAddProductHooks } from "../../hooks/addProductHooks";

export default function addProduct() {
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  const { control, handleSubmit, setValue, reset } = useAddProductHooks();
    ...
    <Container>
      <Box sx={{ display: "flex", gap: 3, justifyContent: "space-between" }}>
        <Box width={"100%"} flex={3} display={"flex"} flexDirection={"column"} gap={5}>
          ...
          <form onSubmit={handleSubmit(submit)}>
            <Box sx={{ display: "flex", gap: 4, flexDirection: "column" }}>
              ...
              <Controller control={control} name="image" render={() => <TextField size="small" type="file" placeholder="Product Photo" inputProps={{ accept: "image/*" }} onChange={handleFileChange} />} />
        ...
        <Box flex={2} textAlign={"center"} display={"flex"} justifyContent={"center"}>
          {previewImage && <img style={{ maxWidth: "20rem", objectFit: "cover" }} src={previewImage} alt="Preview Product" />}
        </Box>
      </Box>
    </Container>
  );
